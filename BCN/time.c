...
char * GetTimeOfNow()
{
    ...
    time(&Timep);
    P = localtime(&Timep);
    sprintf(Time, "%d/%d/%d %s %d:%d:%d",(1900+P->tm_year), (1+P->tm_mon), P->tm_mday, Wday[P->tm_wday], P->tm_hour, P->tm_min, P->tm_sec);
    return Time;
}
...
void timeExamp(void)
{
    ...
    printf("A = %d, b = %d\n", A, b);
    printf("[File:%s, Fun:%s, Line:%d, Time: %s] \n", __FILE__, __func__,  __LINE__, __DATE__, GetTimeOfNow());
