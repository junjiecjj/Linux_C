...
typedef struct DirectionPLOTIQ {
  ...
  unsigned char
      ucDDCid; // 用于区分是哪个，1：调频；
               // 2：扩频；3：扩测，6：7波束调频；7:7波束单频；8:和差波束；9:9波束
...
typedef struct TAP_HJSYS_PARM_MANAGE {
  ...
  unsigned char
      RRFFreq1Flag; //自动捕获有效 0:无此参数1:自动2：数引捕获3：程引捕获
  ...
  double Ypos;
  double Zpos;
  unsigned char AimFlag; //  波控工作方式控制- 等待点位置目标标识，1:位置 2：
                         //  角度   3： 相对时
  ...
  double bpAz
      [65]; // 37或65个模拟波位在阵面系下的方位，相对于捕获时下发的等待点的方位
  double bpEle
      [65]; // 37或65个模拟波位在阵面系下的俯仰，相对于捕获时下发的等待点的俯仰

  unsigned char TargetSure; //相关
  unsigned char TrackingMode; // 当前跟踪方式1：待机     2：手摇    3：指向
                              // 4：数引    5：程引     6：记忆跟踪
  double ScanAz;
  double ScanEle;
