...
******************************************  putchar() **************************************************************************
int putchar(int char) 把参数 char 指定的字符（一个无符号字符）写入到标准输出 stdout中。
...
返回值：如果成功，该函数返回 str。如果发生错误或者到达文件末尾时还未读取任何字符，则返回空指针NULL。
...
********************************************  fputs() *******************************************************************************************
int fputs(const char *str, FILE *stream) 把字符串写入到指定的流 stream 中，但不包括空字符。 返回值：该函数返回一个非负值，如果发生错误则返回 EOF。
...
*******************************************  fopen() **********************************************************
FILE *fopen(const
char *filename, const char *mode) 返回值：该函数返回一个 FILE 指针。否则返回 NULL，且设置全局变量 errno 来标识错误。
...
模式	描述
r	  打开一个已有的文本文件，只允许读取文件。                                                                                                    [文件必须已经存在。                只让读，不让写。]
w	  打开一个文本文件，允许写入文件。如果文件不存在，则会创建一个新文件。在这里，您的程序会从文件的开头写入内容。如果文件存在，则该会被截断为零长度，重新写入。   [如果文件不存在，则会创建一个新文件。不让读, 只让写，从头截断开始写。]
a	  打开一个文本文件，以追加模式写入文件。如果文件不存在，则会创建一个新文件。在这里，您的程序会在已有的文件内容中追加内容。                                [如果文件不存在，则会创建一个新文件。不让读, 只让写, 尾部追加写.]
r+	打开一个文本文件，允许读写文件。文件必须已经存在。读取会从文件的开头开始，写入则默认是追加模式。                                                    [文件必须已经存在。                可以读，从头读，可以写，尾部追加写]
w+	打开一个文本文件，允许读写文件。如果文件已存在，则文件会被截断为零长度，如果文件不存在，则会创建一个新文件。经过测试好像不让读取文件。                    [如果文件不存在，则会创建一个新文件。不让读，只让写，从头截断开始写，]
a+	打开一个文本文件，允许读写文件。如果文件不存在，则会创建一个新文件。读取会从文件的开头开始，写入则只能是追加模式。                                     [如果文件不存在，则会创建一个新文件。可以读，从头读，可以写，尾部追加写。]
...
void C_IO_Mode(void)
{
  ...
  if ((fin = fopen("./IoModeTest.txt", "w+")) == NULL){
      fprintf(stderr, "\n Cannot open the file!!!\n");
      exit(1);
  }
